...
<div x-data="{
    ...
    async search() {
        ...
        @if(auth()->user()->isAdmin() && isset($selectedCompanyId))
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div class="flex flex-col md:flex-row gap-4 items-stretch md:items-center">
            <!-- Admin Company Filter (if admin) -->
            @if(auth()->user()->isAdmin())
            <div class="w-full md:w-64">
                <form method="GET" action="{{ route('debtors.index') }}" id="company-filter-form">
                    <select 
                        name="company_id"
                        onchange="document.getElementById('company-filter-form').submit()"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-base bg-white">
                        <option value="all" {{ $selectedCompanyId === 'all' ? 'selected' : '' }}>All Companies</option>
